...
package body DNS_Client is
   ...
   function Create_DNS_Packet_Name_Record(Question: Parsed_DNS_Question)
   ...
   is
      ...
      function Create_QName_Record(Domain_Section: String)
        return Unbounded_String
      is
         Label_Length: Unsigned_8;
      begin
         Put_Line(Domain_Section);
         return To_Unbounded_String(Domain_Section);
   ...
   begin
      ...
      loop
         ...
         if Element(Question.QName, String_Offset) = '.' then
            Section_Count := Section_Count + 1;
            DNS_Name_Record := DNS_Name_Record &
              Create_QName_Record(Slice(Question.QName, Last_Section_Offset, String_Offset-1));
         ...
         if String_Offset = Length(Question.QName)+1 then
            Section_Count := Section_Count + 1;
            DNS_Name_Record := DNS_Name_Record &
              Create_QName_Record(Slice(Question.QName, Last_Section_Offset, String_Offset-1));
            exit; -- We're done
      ...
      Put_Line("DNS Name had" & Section_Count'Image & " Sections");
